 ...
 */
final class Limit_Stream implements Stream_Interface
{
     ...
     */
    public function __construct(Stream_Interface $stream, int $limit = -1, int $offset = 0)
    {
        $this->stream = $stream;
    ...
    {
        if (null === $length = $this->stream->get_size()) {
            return null;
        }
        if ($this->limit === -1) {
